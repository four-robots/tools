...
export class WebhookManager {
  ...
  async getRepositoryWebhooks(repositoryId: string): Promise<RepositoryWebhook[]> {
    ...
    return result.map(row => ({
      ...
      events: typeof row.events === 'string' ? (() => { try { return JSON.parse(row.events); } catch { return []; } })() : row.events || [],
  ...
  private async getWebhook(webhookId: string): Promise<RepositoryWebhook | null> {
    ...
    return {
      ...
      events: typeof result.events === 'string' ? (() => { try { return JSON.parse(result.events); } catch { return []; } })() : result.events || [],
      isActive: result.is_active,
